...
jjb_cmd = find_executable('jenkins-jobs') or sys.exit('jenkins-job-builder is not found.')
...
for conf_filename in filelist:
    with open(conf_filename) as f:
        ...
        buffer = template.safe_substitute(
            ...
            JJB_TOKEN=os.environ.get('JJB_TOKEN'))
        with open('template.yaml', 'w') as f:
            f.write(buffer)
        try:
            arguments = [jjb_cmd, '--conf', 'jenkins_jobs.ini', '--user', os.environ.get('JJB_USER'), '--password',
                         os.environ.get('JJB_TOKEN'), 'update', 'template.yaml']
            proc = subprocess.Popen(arguments,
                                    stdin=subprocess.PIPE,
                                    stdout=subprocess.PIPE,
                                    universal_newlines=False,
